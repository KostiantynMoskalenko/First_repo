...
Ключ словника — це унікальний ідентифікатор бази даних MongoDB, а значення &mdash це email користувача. Вам необхідно випадково 
відібрати декілька переможців розіграшу.
...
def get_random_winners(quantity, participants):
    if quantity > len(participants):
        return([])
    key_dict = list((participants.keys()))
    random.shuffle(key_dict)
    key_dict = random.sample(key_dict, k=quantity)
    return(key_dict)
