'''Тепер ми потренуємося писати корисні регулярні вирази. Напишіть регулярний вираз для функції find_all_emails, яка буде
 в тексті (параметр text) знаходити всі email та повертати список отриманих з тексту збігів.
...
def find_all_emails(text):
    result = (re.findall(r'[a-zA-Z_.]{1}[a-zA-Z0-9_.]{1,255}[@]{1}\w{1,249}[.]{1}\w{2,3}\b', text))
    return (result)

def find_all_emails1(text):
